...
{ // for testing purposes
    ...
    // if(filter_id_imu == 0) gpio_pin_set(led_dev, PIN0, (int)true);
    // if(filter_id_bms == 0) gpio_pin_set(led_dev, PIN1, (int)true);
    //if(filter_id_mtr == 0) gpio_pin_set(led_dev, PIN2, (int)true);
}
...
void receive_main(void) {
    ...
    while (true) {
        ...
        filter_id_mtr = can_attach_msgq(can_dev, &mtr_queue, &mtr_filter);
         ...
         //recieve_callback();
        int test_queue=k_msgq_get(&bms_queue, &rx_frame, K_NO_WAIT);

        if(test_queue==-ENOMSG)
        {
            k_msleep(150);
